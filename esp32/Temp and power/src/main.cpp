...
#define O_WIFI 22
#define WIFI_ON_TIME 12*60*60*1000
#define WIFI_OFF_TIME 15*60*1000

// #define WIFI_ON_TIME 5*60*1000
// #define WIFI_OFF_TIME 2*60*1000

DHT_Unified dht(DHTPIN, DHTTYPE);

bool heaterOn = false;
bool wifiOn = false;
unsigned long wifiStateTime = millis();
...
void turnHeater(bool turnOn) {
  ...
  // KASASmartPlug *testPlug = kasaUtil.GetSmartPlug("Kitchen");
...
void setup()
{
  ...
  pinMode (O_WIFI, OUTPUT);
  digitalWrite(O_WIFI, HIGH);
  wifiOn = true;
...
void loop()
{
  ...
  Serial.print("Wifi state time: ");
  Serial.println((millis() - wifiStateTime)/1000);

  if (wifiOn) {
    Serial.println("Wifi is On");
    if ((millis() - wifiStateTime) > WIFI_ON_TIME) {
      Serial.println("Turning wifi off");
      digitalWrite(O_WIFI, LOW);
      wifiOn = false;
      wifiStateTime = millis();
    }
  } else {
    Serial.println("Wifi is Off");
    if ((millis() - wifiStateTime) > WIFI_OFF_TIME) {
