...
const char *KASAUtil::cmdSystem = "\"system\":{\"set_relay_state\":{\"state\":";
const char *KASAUtil::cmdContext = "\"context\":{\"child_ids\":[\"";
            ...
            if (FD_ISSET(sock, &rfds))
            {
                ...
                if (len > 500)
                {
                    ...
                    else
                    {
                        ...
                        deviceId = get_sysinfo["deviceId"];
                        relay_state = get_sysinfo["relay_state"];
                        model = get_sysinfo["model"];
                        children = get_sysinfo["children"];
                        ...
                        if (IsStartWith("KP",model))
                        {
                          for (int i=0; i<children.size(); i++) {
                            strcpy(alias, string_value);
                            strcat(alias, " - ");
                            strcat(alias, get_sysinfo["children"][i]["alias"]);
                            strcpy(childId, deviceId);
                            strcat(childId, get_sysinfo["children"][i]["id"]);
                            Serial.println(alias);
                            Serial.println(childId);
                            if (IsContainPlug(alias) == -1)
                            {
                                // New device has been found
                                if (deviceFound < MAX_PLUG_ALLOW)
                                {
                                    ptr_plugs[deviceFound] = new KASASmartPlug(alias, raddr_name, childId);
                        ...
                        } else if (IsStartWith("HS",model))
                        {
                            ...
                            if (IsContainPlug(string_value) == -1)
                            {
                                ...
                                if (deviceFound < MAX_PLUG_ALLOW)
                                {
                                    ptr_plugs[deviceFound] = new KASASmartPlug(string_value, raddr_name, childId);
                                    ptr_plugs[deviceFound]->state = relay_state;
                                    strcpy(ptr_plugs[deviceFound]->model, model);
                                    deviceFound++;
                                } else 
...
void KASASmartPlug::SetRelayState(uint8_t state)
{
  ...
  strcpy(sendbuf, "{");
  if (id[0] != 0x00){
    strcat(sendbuf, KASAUtil::cmdContext);
    strcat(sendbuf, id);
    strcat(sendbuf, "\"]}, ");
  } 
  strcat(sendbuf, KASAUtil::cmdSystem);
  if (state > 0)
  {
    strcat(sendbuf, " 1}}}");
  } else
